...
function Editcustomerwrapper() {
  ...
  const updateDateOfBirth = (value) => {
    ...
    if (value) {
      ...
      if (age < 18) {
        setDateOfBirthError("Age must be 18 years or older");
      }
    }
  };
  ...
  async function getSingleCustomerData(customerId) {
    ...
    await Customerapi.get("get-single-customer-data", {
      ...
      .then((response) => {
        ...
        console.log("update_customer_data", data);
        if (data !== null) {
        ...
        <div className="flex flex-col gap-4 border border-[#ebecef] rounded-xl bg-white p-8">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1 w-full">
              ...
              {phoneCodeError && <p className="mt-1 text-xs text-red-500">{phoneCodeError}</p>}
                  ...
                  {correspondenceState && (
                    <div className="flex flex-col gap-1">
                      ...
                      <Select value={correspondenceCity || undefined} onValueChange={updateCorrespondenceCity}>
                        ...
                        <SelectContent className="border border-gray-200 max-h-[200px]">
                          {correspondenceCityData.map((item) => (
                            <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
          ...
          <div className="flex justify-end mt-auto">
            <Link
              ...
              className={`cursor-pointer text-sm text-white px-4 py-2 rounded bg-[#0083bf] ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
