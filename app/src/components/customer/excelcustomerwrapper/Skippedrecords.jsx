            ...
            <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto w-full">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-sm mt-2 text-gray-600">
                        <span className="font-semibold text-green-600">{insertedCount}</span> customers were successfully imported, but <span className="font-semibold text-red-600">{skippedCount}</span> customers were skipped due to errors.
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {skippedData?.map((item, index) => {
                                    ...
                                    const name = row["First Name"] && row["Last Name"] ? `${row["First Name"]} ${row["Last Name"]}` : (row["First Name"] || row["Last Name"] || 'N/A');
                                    ...
                                    return (
                                        <tr key={index} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3 align-top text-red-600 font-medium whitespace-pre-line">
                                                {item.reason}
                                            </td>
                                        </tr>
                                    );
