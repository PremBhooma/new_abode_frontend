...
function Flattocustomer({ closeFlatToCustomer, refreshGetAllFlats, prefilledData }) {
            ...
            if (prefilledData.customerUid) {
                ...
                const fetchCustomer = async () => {
                    try {
                        ...
                        // toast.error("Could not pre-fill customer details.");
                    }
                };
                fetchCustomer();
            }

            if (prefilledData.flatNo) {
                // Set searched flat and trigger search/select
                // Since getFlatsData is async and sets state, we can try to call it and select
                const fetchFlat = async () => {
                    setSearchedFlat(prefilledData.flatNo);
                    try {
                        // Using search-flats to get the flat object
                        const response = await Flatapi.get(`search-flats`, {
                            params: {
                                flat_no: prefilledData.flatNo,
                                employeeId: employeeId,
                                project_uid: prefilledData.projectUid // Optional if API supports it to be more specific
                            },
                        });

                        if (response?.data?.status === "success" && response?.data?.data?.length > 0) {
                            // Find exact match if possible
                            const flatItem = response.data.data.find(f => f.flat_no == prefilledData.flatNo) || response.data.data[0];
                            // Construct flat object compatible with handleSelectFlat
                            // The search API returns items with label/value? 
                            // Based on handleSelectFlat usage: flat.label, flat.square_feet etc.
                            // The search map in JSX uses item.label, item.value. 
                            // I assume the API returns these or I should match what search returns.
                            // Wait, getFlatsData sets 'flat' state. 
                            // Let's assume response.data.data items have label/value or we construct it.
                            // The search component maps item.label. 
                            // Code check: 
                            // flat.map((item) => ... {item.label})
                            // So the API response objects MUST have 'label'.
                            // If I fetched it from same API, it should have it.

                            handleSelectFlat(flatItem);
                        }
                    } catch (error) {
                        console.error("Error fetching prefilled flat:", error);
                    }
                };
                fetchFlat();
            }
        }
    }, [prefilledData, employeeId]);
