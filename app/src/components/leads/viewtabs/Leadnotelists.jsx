...
import { IconUser, IconClock, IconMessage2, IconDotsVertical } from '@tabler/icons-react';
import Errorpanel from '../../shared/Errorpanel';
import Leadapi from '../../api/Leadapi';

function Leadnotelists({ refreshKey }) {
    ...
    async function getLeadNotes() {
        setIsLoading(true);
        try {
            const response = await Leadapi.get('get-lead-notes', {
                params: { leadId, user_id },
                headers: {
            ...
            {notesData.length > 0 ? (
                        ...
                        >
                            ...
                            <div className="mt-3 pl-[52px] flex items-center justify-end">
                                <span className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">
                                    Clinical Record • #{note.id?.slice(-6).toUpperCase()}
                                </span>
                            </div>
                        </div>
                    ))}
                ...
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200">
                    ...
                    <p className="text-slate-500 text-sm max-w-[280px]">
                        Start documenting lead interactions and important details by adding your first note above.
                    </p>
                </div>
            )}
        </div>
